...
def createThing(self, ThingNum, CUmakeit_IoT_Policy):
    ...
    cert = iot.CfnCertificate(
        self, cert_name, certificate_signing_request=csr, status='ACTIVE')

    # Attach the Certificate to the Thing
    iot.CfnThingPrincipalAttachment(
        self, cert_attachment_ID, principal=cert.attr_arn, thing_name=thing.ref)

    # Attach the Policy to the Certificate
    iot.CfnPolicyPrincipalAttachment(
        self, policy_attachment_ID, principal=cert.attr_arn, policy_name=CUmakeit_IoT_Policy.ref)

    # Add to secrets manager
    private_key = secrets.CfnSecret(
        self, private_key_name, name=private_key_name, secret_string=key)
